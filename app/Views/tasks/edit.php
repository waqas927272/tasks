...
<div class="form-page">
    ...
    <form method="POST" action="<?= url('tasks/' . $task['id']) ?>" class="task-form" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label for="attachments">Attach Files (Documents/Screenshots)</label>
            <input type="file" id="attachments" name="attachments[]" class="form-control" multiple accept="image/*,.pdf,.doc,.docx,.xls,.xlsx">
            <small class="form-text text-muted">Allowed formats: Images (JPEG, PNG, GIF), PDF, Word, Excel. Max size: 10MB per file.</small>
        </div>
        
        <div class="form-actions">
            <button type="submit" class="btn btn-primary">Update Task</button>
            <a href="<?= url('tasks/' . $task['id']) ?>" class="btn btn-secondary">Cancel</a>
        </div>
    </form>
</div>
